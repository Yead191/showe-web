...
const MOCK_VENUES = [
  {
    ...
    location: "Downtown",
    image: "/assets/images/venues/venue1.jpg",
  },
];
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <Tabs defaultValue="programmes" className="w-full">
        <TabsList className="bg-gray-100/50 p-1 rounded-2xl mb-8 flex overflow-x-auto no-scrollbar whitespace-nowrap">
          ...
          <TabsTrigger
            value="venues"
            className="rounded-xl px-4 md:px-8 py-3 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-primary-600 font-black text-[10px] md:text-xs uppercase tracking-widest transition-all"
          >
            Venues
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {MOCK_PROGRAMMES.map((item) => (
              <Link
                ...
                key={item.id}
                className="group bg-gray-50/50 rounded-3xl p-4 border border-transparent hover:border-[#014B52]/10 hover:bg-white hover:shadow-xl transition-all duration-500"
              >
