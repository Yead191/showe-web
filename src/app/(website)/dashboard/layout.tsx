                    ...
                    <aside className="lg:w-80 shrink-0">
                        <div className="lg:sticky lg:top-28 space-y-6">
                            ...
                            <nav className="bg-white lg:bg-white rounded-[24px] lg:rounded-[32px] p-2 lg:p-3 shadow-sm border border-gray-100/50 sticky top-20 z-30 lg:static">
                                <div className="flex lg:flex-col overflow-x-auto lg:overflow-x-visible no-scrollbar gap-1 lg:gap-1">
                                    {sidebarItems.map((item) => {
                                        ...
                                        return (
                                            <Link
                                                key={item.href}
                                                href={item.href}
                                                className={`flex items-center gap-3 lg:gap-4 p-3 lg:p-4 rounded-2xl transition-all duration-300 group shrink-0 ${
                                                    isActive 
                                                    ? "bg-[#014B52] lg:bg-[#014B52]/5 text-white lg:text-[#014B52]" 
                                                    : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
                                                }`}
                                            >
                                                <div className={`hidden lg:flex p-2.5 rounded-xl transition-all duration-300 ${
                                                    isActive ? "bg-[#014B52] text-white" : "bg-gray-100 text-gray-400 group-hover:bg-gray-200 group-hover:text-gray-600"
                                                }`}>
                                                    <item.icon size={18} strokeWidth={2.5} />
                                                </div>
                                                <div className="flex-1">
                                                    <p className="text-sm font-black tracking-tight leading-none lg:mb-1">{item.label}</p>
