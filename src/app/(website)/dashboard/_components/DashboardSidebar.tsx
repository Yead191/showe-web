...
import { User, Heart, Ticket, ChevronRight } from "lucide-react";
          ...
          <div className="flex lg:flex-col lg:overflow-x-visible no-scrollbar gap-1 w-full items-center justify-between lg:justify-start">
            {sidebarItems.map((item) => {
              ...
              return (
                ...
                >
                  <div
                    className={`hidden lg:flex p-2.5 rounded-xl transition-all duration-300 ${
                      isActive
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-black tracking-tight leading-none lg:mb-1 text-nowrap text-center md:text-start">
                      {item.label}
                    </p>
                    <p className="hidden lg:block text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                      {item.description}
                    </p>
                  </div>
                  <ChevronRight
                    size={14}
                    className={`hidden lg:block transition-transform duration-300 ${isActive ? "translate-x-0 opacity-100" : "-translate-x-2 opacity-0"}`}
                  />
                </Link>
              );
            })}
