...
export function VenueHero({ venue }: { venue: Venue }) {
  ...
  const logoImage = venue.logo
    ? getImageUrl(venue.logo)
    : venue.cover_image
      ? getImageUrl(venue.cover_image)
      : null;

  const locationText =
    [venue.address_line1, venue.city, venue.country]
      .filter(Boolean)
      .join(", ") || "Location";
    ...
    <section id="banner" className="relative w-full">
      ...
      <div className="container mx-auto px-4 -mt-20 md:-mt-24 relative z-10">
        <div className="flex flex-col md:flex-row items-start md:items-end gap-6 md:gap-10 pb-6 border-b border-gray-100">
          {/* Venue Logo / Avatar */}
          <div className="relative h-32 w-32 md:h-44 md:w-44 rounded-3xl overflow-hidden border-4 border-white bg-primary-600 shadow-2xl shrink-0 flex items-center justify-center">
            {logoImage ? (
              <Image
                src={logoImage}
                alt={venue.name}
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            >
              <Heart
                className={`h-5 w-5 ${isFavorited ? "fill-white text-white" : "text-gray-700"}`}
              />
