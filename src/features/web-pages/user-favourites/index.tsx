                ...
                >
                  <Link href={`/events/${event._id}`} className="block">
                    ...
                    <div className="p-5 space-y-2">
                      {event.category && (
                        <p className="text-[10px] font-black uppercase tracking-widest text-accent-400">
                          {event.category}
                        </p>
                      )}
                      <h3 className="text-base font-black text-gray-900 line-clamp-2 group-hover:text-accent-400 transition-colors">
                        {event.title}
                      </h3>
                      {event.address && (
                        <div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium">
                          <MapPin className="h-3.5 w-3.5 text-accent-400 shrink-0" />
                          <span className="line-clamp-1">{event.address}</span>
                        </div>
                      )}
                      {event.event_date && (
                        <div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium">
                          <Calendar className="h-3.5 w-3.5 text-accent-400 shrink-0" />
                          <span>
                            {format(
                      ...
                      {event.price != null && (
                        <p className="text-lg font-black text-accent-400">
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
      ...
      <Link
        href={href}
        className="text-sm font-black uppercase tracking-widest text-primary-600 underline underline-offset-4 hover:text-accent-400 transition-colors"
      >
        {cta}
      </Link>
    </div>
