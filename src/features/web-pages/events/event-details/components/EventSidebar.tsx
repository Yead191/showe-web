...
export function EventSidebar({ event, user }: EventSidebarProps) {
  const handleComingSoon = (feature: string) => {
    toast.info(`${feature} feature will be available soon!`, {
      description:
        "We are currently working on integrating this with the venue's ticketing system.",
    });
  };
        ...
        <div className="p-6 md:p-8 space-y-8">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-4xl font-black text-accent-400 tracking-tight">
                £{event.price}
              </p>
            </div>
          ...
          {event.programme?._id && (
            <ProgrammePurchase
              ...
              user={user}
              isPurchased={event?.is_already_programme_purchased}
            />
          )}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs font-bold text-gray-500">
                  {(event.author.followers_count ?? 0).toLocaleString()}{" "}
                  Followers
                </p>
              </div>
              <Button
                variant="outline"
                size="sm"
                className="rounded-full text-[10px] font-black uppercase tracking-widest border-gray-200 hover:border-accent-400 hover:text-accent-400 transition-colors"
              >
