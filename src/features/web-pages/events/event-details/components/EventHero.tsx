...
export function EventHero({ event }: { event: EventDetail }) {
  ...
  return (
    <section
      id="banner"
      className="relative w-full h-[90vh] md:h-[65vh] lg:h-screen 2xl:h-[85vh] pt-18"
    >
      {/* Background Image */}
      <div className="absolute inset-0 z-0">
        <Image
          src={
            ...
            "/assets/bg/programmes/theatre.png"
          }
          alt={event.title}
          fill
          ...
          priority
        />
        <div className="absolute inset-0 bg-linear-to-t from-primary-600 via-primary-600/40 to-black/80" />
      </div>
      ...
      <div className="container relative z-10 h-full mx-auto px-4 flex flex-col justify-end pb-12 lg:pb-4 2xl:pb-16">
        ...
        {/* Phone (centered) */}
        <div className="flex justify-center items-end pb-10 sm:pb-0">
          {event.programme?.cover_image && (
            <div className="relative w-70 h-110  md:h-146.25 lg:w-74 2xl:w-87.5 lg:h-[84vh] 2xl:h-155 rounded-[3rem] bg-neutral-900 p-3 shadow-2xl shadow-black/60 ring-1 ring-white/15">
              {/* Side buttons */}
              <span className="absolute -left-0.75 top-28 h-16 w-0.75 rounded-l bg-neutral-700 hidden sm:block" />
              <span className="absolute -right-0.75 top-24 h-12 w-0.75 rounded-r bg-neutral-700 hidden sm:block" />
              <span className="absolute -right-0.75 top-40 h-20 w-0.75 rounded-r bg-neutral-700 hidden sm:block" />
