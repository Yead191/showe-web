  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-4/3 overflow-hidden shrink-0">
        ...
        />
        <button
          className={`absolute top-4 right-4 bg-white/90 backdrop-blur-sm p-2 rounded-full hover:bg-white transition-all shadow-sm ${event.isFavorited ? "text-red-500" : "text-gray-400 hover:text-red-500"}`}
        >
          <Heart
            className={`h-5 w-5 ${event.isFavorited ? "fill-current" : ""}`}
          />
        </button>
      </div>
        ...
        <div className="mt-auto pt-4 space-y-4">
          ...
          <div className="flex items-center justify-between border-t border-gray-50 pt-4">
            <div className="flex items-center gap-2">
              {event?.someInterestPeopsle?.length &&
                event?.someInterestPeopsle?.length > 0 && (
