...
export function StepCommissionBreakdown() {
  ...
  const rows = [
    ...
    {
      label: "SHOWE commission (10%)",
      value: `– ${fmt(c.showeCommission)}`,
      tone: "deduct",
    },
    {
      label: "VAT on commission (20%)",
      value: `– ${fmt(c.vatOnCommission)}`,
      tone: "deduct",
    },
  ...
  return (
    ...
    >
      <div className="bg-linear-to-br from-primary-600 to-[#013138] rounded-3xl p-8 text-white shadow-2xl shadow-primary-600/20">
            ...
            >
              ...
              <span
                className={`font-mono text-base ${
                  r.tone === "deduct" ? "text-rose-300" : "text-white"
                }`}
              >
                {r.value}
              </span>
        ...
        <div className="mt-6 pt-6 border-t-2 border-accent-400/30 flex items-center justify-between">
          <div>
            <p className="text-xs text-accent-400 uppercase font-bold tracking-wider">
              You receive
            </p>
          ...
          <span className="text-3xl md:text-4xl font-bold text-accent-400 font-mono">
      ...
      <div className="mt-5 p-4 rounded-xl bg-amber-50 border border-amber-200">
        <p className="text-sm text-amber-900">
          <span className="font-bold">
            Minimum sale price: {fmt(c.minimumSalePrice)}.
          </span>{" "}
          To keep the maths reasonable for everyone, programmes must be priced
          at £2 or above when sold.
        </p>
      </div>
    </StepShell>
  );
