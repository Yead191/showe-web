...
export function StepOrgCapture({ state, dispatch }: Props) {
  const o = state.org;
  const set = (patch: Partial<OrgCapture>) =>
    dispatch({ type: "SET_ORG", value: patch });
    ...
    <div className={`space-y-1.5 ${fullWidth ? "md:col-span-2" : ""}`}>
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="pl-10 h-11 border-slate-200 focus:border-primary-600 focus:ring-primary-600/20"
        />
      </div>
    </div>
  );
